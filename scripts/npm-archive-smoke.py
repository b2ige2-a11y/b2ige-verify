...
m = json.loads((root / 'release/release-manifest.json').read_text())
out = root / 'release/artifacts'
with tempfile.TemporaryDirectory(prefix='b2ige-npm-smoke-') as tmp:
    stage = pathlib.Path(tmp)
    npm_name = next(name for name in m['artifact_sha256'] if name.endswith('.tgz'))
    for name in [npm_name, f'b2ige-{m["version"]}-{m["built_target"]}.tar.gz']:
        assert hashlib.sha256((out / name).read_bytes()).hexdigest() == m['artifact_sha256'][name]
        with tarfile.open(out / name) as t: t.extractall(stage, filter='data')
