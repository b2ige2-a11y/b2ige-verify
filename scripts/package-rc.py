...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='b2ige-package-') as temp:
        ...
        for p in public:
            ...
            if rel.parts[0] in {'docs', 'examples', 'schemas', 'skills', 'npm', 'benchmarks', 'release'} or (len(rel.parts) == 1 and p.suffix in {'.md', '.txt'}) or p.name in {'LICENSE', 'ci-verify.py', 'ci-summary.py', 'ci_summary.py', 'diff-verify.py', 'setup.py', 'demo.py', 'demo-all.sh', 'demo-behavior.sh', 'demo-sideeffect.sh', 'demo-blindtest.sh', 'readme-bench.py'}:
