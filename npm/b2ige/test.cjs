...
const versionProbe = 'if [ "$1" = "--version" ]; then echo "verify-cli 0.3.0"; exit 0; fi\n';
...
test('bundled pin works and altered manifest version is refused', () => {
  ...
  try {
    ...
    const manifest={schema_version:'2',version:'0.3.0',release_repository:null,platforms:{[platform]:{sha256:trustedEnv(f).B2IGE_BINARY_SHA256}}};
